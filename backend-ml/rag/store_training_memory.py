...
def store_training_memory(user_id : str ,  data : dict):
    ...
    text = f"""
        ...
        Top Features: {data.get('top_features') if isinstance(data.get('top_features'), str) else (', '.join([f"{name} ({score:.4f})" if isinstance(score, (int, float)) else f"{name} ({score})" for name, score in data.get('top_features')]) if data.get('top_features') else 'N/A')}
